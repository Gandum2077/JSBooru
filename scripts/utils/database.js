...
function insertFavorite({ site, id, preview_url, info }) {
  const db = $sqlite.open(databaseFile);
  db.update({
  ...
  db.update({
    ...
    args: [site, id, preview_url, JSON.stringify(info)]
  });
  $sqlite.close(db);
}

function deleteFavorite({ site, id }) {
  const db = $sqlite.open(databaseFile);
  db.update({
    sql: "DELETE FROM favorites WHERE site=? AND id=?",
    args: [site, id]
...
function handleQuery({ page = 1, site = null }) {
  ...
  return {
    clause:
      "SELECT DISTINCT * FROM favorites" +
      where_clause +
      sort_clause +
      limit_clause,
    args: args
...
function searchFavorites({ site = null, page = 1 }) {
