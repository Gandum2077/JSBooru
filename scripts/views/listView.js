...
} = require("scripts/booru/generator");
const {
...
function refreshAccessoryViewMatrixData() {
  $("accessoryViewMatrix").data = [
    "\u{1f4cb}Clipboard",
    ...constants.userConfig.search_history
  ].map(n => {
    return { button: { title: n } };
  });
}
  ...
  const titleView = {
      ...
      },
      {
          ...
          accessoryView: {
            ...
            events: {
              itemSize: function(sender, indexPath) {
                ...
                const adjustedWidth =
                  Math.min(Math.max(40, textWidth), 290) + 10;
        ...
        events: {
          didBeginEditing: function(sender) {
            refreshAccessoryViewMatrixData();
          },
    ...
    events: {
      layoutSubviews: sender => {
        ...
        input.frame = $rect(0, 0, sender.size.width, sender.size.height);
      }
    }
  };
  return titleView;
...
async function load({ site, tags = [], random = false, startPage = 1 }) {
  SITE = site;
  TAGS = tags;
  RANDOM = random;
  STARTPAGE = startPage;
