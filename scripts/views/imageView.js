      ...
      touchesEnded: function (sender, location, locations) {
        if (sender.super.zoomScale !== 1) {
          ...
          if (location.y <= 100 || sender.frame.height - 100 <= location.y) {
            FOOTER_HIDDEN = false
            $("footerView").hidden = FOOTER_HIDDEN
          } else if (100 < location.y && location.y <= sender.frame.height - 100) {
            const newIndex = (location.y <= sender.frame.height / 2) ?
              Math.max(INDEX - 1, 0) :
    ...
    events: {
      ready: async function (sender) {
        ...
        sender.get("scroll").get("contentView").frame = $rect(
          0, 0, sender.get("scroll").frame.width,
          sender.get("scroll").frame.height
        )
        sender.get("scroll").get("contentView").add(imageView)
      }
    }
  }
...
function refresh() {
  ...
  mpv.get("scroll").get("contentView").get("image").src = getSrc()
